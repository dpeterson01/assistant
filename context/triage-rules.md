...
- From anyone in chain-of-command UP (Heather, Curtis, Ruhi, or above), addressed To: you with ≤5 recipients
- From engineering counterparts at Heather's level (Sean and other M2-band partners on active initiatives)
- From direct reports OR cross-org skip-down DMs to you 1:1 (e.g., Osmond), with ask/decision/escalation tone
...
- **Aging boost**: unread 2+ business days → escalate one tier. Sean/M2-band use 1-business-day boost.

### 🟡 MEDIUM (review today)
- From PM peers under Heather (Sonia, Mark). HIGH if there's an ask, deadline, or @mention.
- From people named in `priorities.md` as active cross-team partners
- From people one level below Derek's PM-peer band (e.g., Karin) when addressed directly
- Cross-org partners on active initiatives (DevDiv contacts) when addressed to you
- FYI threads where Derek is explicitly To: or @cc'd with needed context
...
Classify per-thread, not per-message. If a MEDIUM thread gets a reply from a HIGH-tier sender (Heather, Curtis, Ruhi, Sean), the entire thread re-classifies as HIGH.

## Action Item Extraction

For every HIGH or MEDIUM item, apply these tests before creating a task:

1. **Explicit ask test**: Direct request, question, decision, or deliverable addressed to Derek? Language like "can you", "please", "I need you to", "your thoughts on"? If no explicit ask: no task. Sharing links/resources without a clear ask does not qualify.
2. **Announcement filter**: Kickoff emails, announcements, FYIs, updates where Derek has no named role or ask → no task. Briefing context only.
...
4. **Group meeting ask filter**: Only create a task if: (a) request explicitly @mentions Derek, AND (b) no visible response already addressing it.
5. **Unaccepted offer filter**: If Derek offered to help but no explicit acceptance or follow-up, do not create a task.
...
Hard exclusions from `/draft-message` Step 2.5 apply (Curtis, Father Francisco, brand-new external contacts, sensitivity flags). Never auto-send. Log auto-saved drafts to `assistant/data/state/auto-drafts.log`.
